...
int booknum = 0;//全局变量（全局变量会在局部被改变）    书本种数
 ...
 *      @notice      : None
 */
void printbook ( Book *bookpack )//输出
{
	printf ( "\t\t--------------------------\n");
	printf ( "\t\t图书编号：%d\n", bookpack->book_id );
	printf ( "\t\t图书名：%s\n", bookpack->book_name );
 ...
 *      @notice      : None
 */
Book *lookupbookname ( char *bookname, int id )//找书本链的对应的书名
{
	Book *bookpack;
	for ( bookpack = head; bookpack != NULL; bookpack = bookpack->next )
	{
		...
		{
			return bookpack;//有就返回
		}
	}
	return NULL;//无就返回空
}
Book *lookupauthorname ( char *authorname, int id )//找书本链的对应的著者名
{
	Book *bookpack;
	for ( bookpack = head; bookpack != NULL; bookpack = bookpack->next )
	{
...
int borrowbook()
{
	...
	if ( ( bookpack = lookupbookname ( namep = strdup ( bookname ), 0 ) ) == NULL )//strdup拷贝函数
	{
		printf ( "\t\t图书 《 %s 》不存在\n", namep );
		fflush ( stdin );
		return ERROR;
...
int returnbook()
{
	...
	Book *bookpack;
	char *namep;
	if ( ( bookpack = lookupbookname ( namep = strdup ( bookname ), 0 ) ) == NULL )//strdup查找
	{
		fflush ( stdin );
		printf ( "\t\t此图书并不是馆内图书\n" );
		return SUCCESS;
